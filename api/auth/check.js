...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!authHeader.startsWith('Bearer ') && !authHeader.startsWith('bearer ')) {
      console.error('Auth check: Invalid authorization header format:', authHeader.substring(0, 20))
      return res.status(401).json({ 
        ok: false, 
        authenticated: false,
    ...
    try {
      ...
      console.log('Auth check: Token verified successfully for user:', decoded.username)
